...
const BACKTEST_SCREENSHOTS_BUCKET = 'backtest-screenshots'
...
export async function uploadBacktestScreenshot(
  ...
  const extension = file.name.split('.').pop()
  const filename = `${crypto.randomUUID()}${extension ? `.${extension}` : ''}`
  const path = `${userId}/${backtestId}/${filename}`

  const { error: uploadError } = await supabase.storage
    .from(BACKTEST_SCREENSHOTS_BUCKET)
    .upload(path, file)

  if (uploadError) throw uploadError

  const { data } = supabase.storage.from(BACKTEST_SCREENSHOTS_BUCKET).getPublicUrl(path)
  return data.publicUrl
}

function extractBacktestScreenshotPath(storageUrl: string): string | null {
  const marker = `/${BACKTEST_SCREENSHOTS_BUCKET}/`
  const markerIndex = storageUrl.indexOf(marker)
  if (markerIndex === -1) return null
  return storageUrl.slice(markerIndex + marker.length)
}

export async function getSignedBacktestScreenshotUrl(
  supabase: SupabaseClient<Database>,
  storageUrl: string
): Promise<string | null> {
  const path = extractBacktestScreenshotPath(storageUrl)
  if (!path) return null
...
export async function deleteBacktestScreenshotFile(
  supabase: SupabaseClient<Database>,
  storageUrl: string
): Promise<void> {
  const path = extractBacktestScreenshotPath(storageUrl)
  ...
  const { error } = await supabase.storage.from(BACKTEST_SCREENSHOTS_BUCKET).remove([path])
  if (error) throw error
}
