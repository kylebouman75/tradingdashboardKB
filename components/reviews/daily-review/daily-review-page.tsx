...
import { DailyReviewOutput } from '@/components/reviews/daily-review/daily-review-output'
...
export function DailyReviewPage({ date }: { date: string }) {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      const [existingReview, preMarket, postMarket, trades] = await Promise.all([
        getDailyReview(supabase, user.id, date),
  ...
  React.useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        await updateDailyReviewContent(supabase, review.id, outputText)
        setSaveStatus('saved')
      } catch {
        setSaveStatus('error')
      }
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch('/api/engines/daily-review', {
        method: 'POST',
      ...
      const created = await createDailyReview(supabase, userId, date, body.content as string)
  ...
  async function handleApprove() {
    ...
    setIsApproving(true)
    try {
      const approved = await approveDailyReview(supabase, review.id, outputText)
      setReview(approved)
    } finally {
  ...
  const insufficientData = summary.tradeCount === 0 && !summary.hasPostMarket
    ...
    <div className="space-y-6">
      ...
      {insufficientData && (
        <p className="text-sm text-amber-400">
          Onvoldoende data voor analyse — log eerst trades en vul Post-Market in.
        </p>
      )}
          ...
          {review.status === 'draft' && (
            <div className="flex items-center justify-between rounded-md border bg-muted/30 px-4 py-3">
              ...
              <Button onClick={handleApprove} disabled={isApproving}>
                {isApproving ? 'Bezig...' : 'Analyse goedkeuren'}
              </Button>
            </div>
          )}
        </div>
